...
class NERDataLoader(object):
    ...
    def collate_fn(features):
        ...
        pseudo = torch.tensor([f.pseudo for f in features], dtype=torch.long) if None not in features else None 

        tensors = [input_ids, input_mask, labels, split_to_ori, example_ids, pseudo]
        return tensors
    ...
    def get_features(self, data_sign):
        ...
        if os.path.exists(cache_path) and self.data_cache:
            ...
            # 生成数据
            features = convert_examples_to_features(self.params, examples, self.tokenizer,data_sign,greed_split=False)
            # save data
            if self.data_cache:
    ...
    def get_dataloader(self, data_sign="train"):
        ...
        if data_sign == "train":
            ...
            raise ValueError("please notice that the data can only be train/val/test/pseudo !!")
        return dataloader
