...
def train(model, data_iterator, optimizer, params, epoch ,args):
    ...
    for step, _ in enumerate(t):
        ...
        input_ids, input_mask, labels, _, _ ,pseudo= batch

        # compute model output and loss
        loss = model(input_ids, attention_mask=input_mask, labels=labels,pseudo=pseudo, epoch=epoch ,args=args)
...
def train_and_evaluate(model, params, args,restore_file=None,):
    ...
    for epoch in range(1, args.epoch_num + 1):
        ...
        train(model, train_loader, optimizer, params, epoch ,args)
        ...
        improve_loss = best_val_loss - val_loss
        ...
        if (patience_counter > params.patience_num and epoch > params.min_epoch_num) or epoch == args.epoch_num or (patience_counter_loss > params.patience_num and epoch > params.min_epoch_num):
