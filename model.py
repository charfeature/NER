    ...
    def get_dym_layer(self, outputs):
        ...
        if self.fusion_layers != 0:
            ...
            sequence_output = torch.sum(hidden_stack * self.dym_weight,
                                    dim=0)  # (bs, seq_len, hs)
        elif self.fusion_layers == 0:
            sequence_output = outputs[fusion_idx][-1]
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if pseudo is not None:
                ...
                if pseudo_nums == 0:
                    loss_0 = loss.mean()
                    loss_1 = (rate*pseudo*loss).sum()
                else:
                    if total_nums == pseudo_nums:
                        loss_0 = 0
                    else:
                        loss_0 = ((1 - pseudo) * loss).sum() / (total_nums - pseudo_nums)
                    loss_1 = (rate*pseudo*loss).sum() / pseudo_nums

                tokens_loss = loss_0 + loss_1
            
            else:
                tokens_loss = loss.sum() / batch_size
