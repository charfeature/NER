...
class InputExample(object):
    ...
    def __init__(self, sentence, tag,is_pseudo=None):
    ...
    if data_sign == 'train':
        with open(data_dir / f'{data_sign}/sentences.txt', "r", encoding='utf-8') as f_sen, \
                open(data_dir / f'{data_sign}/tags.txt', 'r', encoding='utf-8') as f_tag,\
                    open(data_dir / f'{data_sign}/is_pseudo.txt', 'r', encoding='utf-8') as f_is_pseudo:
            for sen, tag, is_pse in zip(f_sen, f_tag,f_is_pseudo):
                example = InputExample(sentence=sen.strip().split(' '), tag=tag.strip().split(' '),is_pseudo =is_pse.strip() )
...
def convert_examples_to_features(params, examples, tokenizer,data_sign,pad_sign=True, greed_split=True):
    ...
    for (example_idx, example) in enumerate(examples):
        ...
        for text, start in zip(sub_texts, starts):
            ...
            if data_sign in ['train']:
                pseudo = int(example.is_pseudo)
            else:
                pseudo = 0
